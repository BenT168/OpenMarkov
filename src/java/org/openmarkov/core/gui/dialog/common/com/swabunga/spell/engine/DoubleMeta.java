...
class DoubleMeta implements Transformator {
	...
	private static final String[] list26 = { "CIO", "CIE", "CIA", "" };
	...
	private static final String[] list49 = { "IER ", "" };
	...
	private static final String[] list96 = { "ZO", "ZI", "ZA", "" };
		...
		while (current < length) {
			...
			case 'G':
				if (in.charAt(current + 1) == 'H') {
					...
					if ((current > 1)
						...
						|| ((current > 2) && stringAt(
							in, current - 3, 1, list34))
						|| ((current > 3) && stringAt(
							in, current - 4, 1, list35))) {
						current += 2;
						break;
					} else {
				...
				if ((current == 0)
					&& ((in.charAt(current + 1) == 'Y') || stringAt(
						in, current + 1, 2, list39))) {
					MetaphAdd(primary, 'K');
					current += 2;
					break;
				}
			...
			case 'L':
				if (in.charAt(current + 1) == 'L') {
					if (((current == (length - 3)) && stringAt(
						in, current - 1, 4, list56))
						|| ((stringAt(in, last - 1, 2, list57) || stringAt(
							in, last, 1, list58)) && stringAt(
							in, current - 1, 4, list59))) {
						MetaphAdd(primary, 'L');
						current += 2;
						break;
					}
			...
			case 'W':
				...
				if ((current == 0)
					&& (isVowel(in, current + 1, length) || stringAt(
						in, current, 2, list89))) {
					MetaphAdd(primary, 'A');
				}
